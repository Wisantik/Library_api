...
class ChangePasswordSerializer(serializers.ModelSerializer):
    ...
    # ANCHOR - после validate_ указываем что валидируем и value примет это значение
...
class MeUpdateSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        # проверка наличия профила
        profile_data = validated_data.pop(
            'profile') if 'profile' in validated_data else None

        with transaction.atomic():  # если что-то сломается на половине занесения данных в базу то ваабще всё откатится
    ...
    def _update_profile(self, profile, data):  # внутренняя функция
        profile_serializer = ProfileUpdateSerializer(
            instance=profile, data=data, pertial=True
        )
